from binutils import *

def apply_transformation(T, d):
	""" Applique la transformation T aux données d. """

	fd = [0]*len(T)

	for i in range(len(T)):
		fd[i] = d[T[i]-1]

	return fd

def apply_sbox(n, d):
	""" Applique la S-Box n° (n+1) aux données d. """

	return SBXb[n][binval(d)]

E = [32,1,2,3,4,5,4,5,6,7,8,9,8,9,10,11,12,13,12,13,14,15,16,17,16,17,18,19,20,21,20,21,22,23,24,25,24,25,26,27,28,29,28,29,30,31,32,1]
P = [16,7,20,21,29,12,28,17,1,15,23,26,5,18,31,10,2,8,24,14,32,27,3,9,19,13,30,6,22,11,4,25]

IP = [58,50,42,34,26,18,10,2,60,52,44,36,28,20,12,4,62,54,46,38,30,22,14,6,64,56,48,40,32,24,16,8,57,49,41,33,25,17,9,1,59,51,43,35,27,19,11,3,61,53,45,37,29,21,13,5,63,55,47,39,31,23,15,7]
FP = [40,8,48,16,56,24,64,32,39,7,47,15,55,23,63,31,38,6,46,14,54,22,62,30,37,5,45,13,53,21,61,29,36,4,44,12,52,20,60,28,35,3,43,11,51,19,59,27,34,2,42,10,50,18,58,26,33,1,41,9,49,17,57,25]

PC1_l = [57,49,41,33,25,17,9,1,58,50,42,34,26,18,10,2,59,51,43,35,27,19,11,3,60,52,44,36]
PC1_r = [63,55,47,39,31,23,15,7,62,54,46,38,30,22,14,6,61,53,45,37,29,21,13,5,28,20,12,4]
PC2 = [14,17,11,24,1,5,3,28,15,6,21,10,23,19,12,4,26,8,16,7,27,20,13,2,41,52,31,37,47,55,30,40,51,45,33,48,44,49,39,56,34,53,46,42,50,36,29,32]

KSR = [1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1]

SBX = [
	[14,0,4,15,13,7,1,4,2,14,15,2,11,13,8,1,3,10,10,6,6,12,12,11,5,9,9,5,0,3,7,8,4,15,1,12,14,8,8,2,13,4,6,9,2,1,11,7,15,5,12,11,9,3,7,14,3,10,10,0,5,6,0,13],
	[15,3,1,13,8,4,14,7,6,15,11,2,3,8,4,14,9,12,7,0,2,1,13,10,12,6,0,9,5,11,10,5,0,13,14,8,7,10,11,1,10,3,4,15,13,4,1,2,5,11,8,6,12,7,6,12,9,0,3,5,2,14,15,9],
	[10,13,0,7,9,0,14,9,6,3,3,4,15,6,5,10,1,2,13,8,12,5,7,14,11,12,4,11,2,15,8,1,13,1,6,10,4,13,9,0,8,6,15,9,3,8,0,7,11,4,1,15,2,14,12,3,5,11,10,5,14,2,7,12],
	[7,13,13,8,14,11,3,5,0,6,6,15,9,0,10,3,1,4,2,7,8,2,5,12,11,1,12,10,4,14,15,9,10,3,6,15,9,0,0,6,12,10,11,1,7,13,13,8,15,9,1,4,3,5,14,11,5,12,2,7,8,2,4,14],
	[2,14,12,11,4,2,1,12,7,4,10,7,11,13,6,1,8,5,5,0,3,15,15,10,13,3,0,9,14,8,9,6,4,11,2,8,1,12,11,7,10,1,13,14,7,2,8,13,15,6,9,15,12,0,5,9,6,10,3,4,0,5,14,3],
	[12,10,1,15,10,4,15,2,9,7,2,12,6,9,8,5,0,6,13,1,3,13,4,14,14,0,7,11,5,3,11,8,9,4,14,3,15,2,5,12,2,9,8,5,12,15,3,10,7,11,0,14,4,1,10,7,1,6,13,0,11,8,6,13],
	[4,13,11,0,2,11,14,7,15,4,0,9,8,1,13,10,3,14,12,3,9,5,7,12,5,2,10,15,6,8,1,6,1,6,4,11,11,13,13,8,12,1,3,4,7,10,14,7,10,9,15,5,6,0,8,15,0,14,5,2,9,3,2,12],
	[13,1,2,15,8,13,4,8,6,10,15,3,11,7,1,4,10,12,9,5,3,6,14,11,5,0,0,14,12,9,7,2,7,2,11,1,4,14,1,7,9,4,12,10,14,8,2,13,0,15,6,12,10,9,13,0,15,3,3,5,5,6,8,11]
]

SBXb = [[int_to_bin(k) for k in s] for s in SBX] # ouhla c'est pas propre

def feistel_scheme(dl, dr, F, n):
	""" Applique un schéma de Feistel (n tours) aux blocs de données
	dl (gauche) et dr (droite), en utilisant la fonction F. """

	l = dl
	r = dr

	for i in range(n):
		l = xor(l, F(r))
		l, r = r, l

	return (r, l)

ENCRYPT = 0
DECRYPT = 1

def key_scheduler(key, action=ENCRYPT):
	if action == ENCRYPT:
		l, r = apply_transformation(PC1_l, key), apply_transformation(PC1_r, key)
		for k in KSR:
			l, r = rot(l, k), rot(r, k)
			yield apply_transformation(PC2, l + r)
	else:		
		for k in reversed([k for k in key_scheduler(key)]):
			yield k
		
def feistel_function(hb, subkey):
	""" Applique la fonction de Feistel au demi-bloc hb, avec la
	sous-clé subkey. """

	d = xor(apply_transformation(E, hb), subkey)
	
	d2 = []

	for i in range(0, 8):
		d2 += apply_sbox(i, d[6*i:6*(i+1)])
	
	return apply_transformation(P, d2)

def std_des(b, key, action=ENCRYPT):
	""" Applique DES 16 tours au bloc de données b (64 bits), avec la clé
	key (64 bits). La fonction ne vérifie pas les 8 bits de parité de la
	clé. """

	ipb = apply_transformation(IP, b)
	
	il = ipb[0:32]
	ir = ipb[32:]
	
	ks = key_scheduler(key, action)
	
	fl, fr = feistel_scheme(il, ir, lambda hb: feistel_function(hb, next(ks)), 16)

	return apply_transformation(FP, fl + fr)
